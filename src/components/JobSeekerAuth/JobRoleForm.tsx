...
const JobRoleForm: React.FC<JobRoleFormProps> = React.memo(({ formData, updateFormData, onNext, onBack }) => {
  const jobRoles = [
  ...
  const {
    ...
    defaultValues: { jobRole: formData.jobRole },
  });
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {jobRoles.map((role) => (
            <Controller
              ...
              render={({ field }) => (
                <div
                  onClick={() => field.onChange(role.name)}
                  ...
                  role="button"
                  tabIndex={0}
                  onKeyDown={(e) => e.key === 'Enter' && field.onChange(role.name)}
        ...
        {errors.jobRole && <p className="text-sm text-red-500">{errors.jobRole.message}</p>}
